...
#
#  TOOLS: ${TOOL_PATHS[nmap]}, ${TOOL_PATHS[curl]}, ${TOOL_PATHS[wget]}
...
run_b2() {
    ...
    if [[ -n "$gw_open_ports" ]]; then
        ...
        while IFS= read -r line; do
            ...
            local protocol=$(echo "$line" | awk -F'/' '{print $2}' | awk '{print $1}')
    ...
    while IFS= read -r target; do
        [[ -z "$target" ]] && continue
        local c_ip port
        local c_ip=$(echo "$target" | cut -d: -f1)
        ...
        for scheme in "https" "http"; do
            ...
            local http_code
            local http_code=$(${TOOL_PATHS[curl]} -sk -o "$response_file" -w "%{http_code}" \
            ...
            if [[ "$http_code" =~ ^[1-5][0-9][0-9]$ ]]; then
                ...
                local page_title
                local page_title=$(grep -ioP '(?<=<title>).*?(?=</title>)' "$response_file" 2>/dev/null | head -1 | xargs)
                [[ -z "$page_title" ]] && page_title="(No title)"

                # Extract server header
                local server_header
                local server_header=$(${TOOL_PATHS[curl]} -skI --connect-timeout 5 --max-time 10 "$url" 2>/dev/null | grep -i "^server:" | head -1 | sed 's/server: //i' | xargs)
    ...
    for target_ip in "$GATEWAY_IP" "${unique_candidates[@]}"; do
        for path in "${cisco_paths[@]}" "${aruba_paths[@]}"; do
            for scheme in "https" "http"; do
                ...
                local http_code
                local http_code=$(${TOOL_PATHS[curl]} -sk -o /dev/null -w "%{http_code}" \
                    --connect-timeout 3 --max-time 5 \
                    "$url" 2>/dev/null || echo "000")

                if [[ "$http_code" =~ ^(200|301|302|401|403)$ ]]; then
                    local page_title
                    local page_title=$(${TOOL_PATHS[curl]} -sk --connect-timeout 3 --max-time 5 "$url" 2>/dev/null | grep -ioP '(?<=<title>).*?(?=</title>)' | head -1 | xargs)
    ...
    if command -v snmpwalk &>/dev/null; then
        for community in "public" "private"; do
            local snmp_result
            local snmp_result=$(timeout 5 ${TOOL_PATHS[snmpwalk]} -v2c -c "$community" "$GATEWAY_IP" system 2>/dev/null | head -5)
    ...
    if [[ $exposed_count -gt 0 ]]; then
        ...
        local result_summary="${exposed_count} management service(s) accessible from target WiFi. ${web_count} web management interface(s) found. "
        ...
        local recommendations="1) Block all management ports (SSH:22, Telnet:23, HTTP:80/8080/8443, HTTPS:443, SNMP:161) from target VLAN using ACLs. 2) Restrict WLC management to a dedicated management VLAN. 3) Implement management ACLs on the WLC itself. 4) Disable HTTP and use HTTPS-only for remaining management access."
    else
        local result_summary="No management services accessible from target WiFi. Gateway and infrastructure devices properly restrict management access from untrusted networks."
        local recommendations="Continue monitoring. Ensure management ACLs are maintained during firmware updates."
    fi

    local result_json
    evidence_register_file "b2_gateway_scan.nmap"
